...
/** Exported so the editor can cap a slug it derives rather than mint an invalid one. */
...
export function detailPath(type, slug) {
  ...
  if (!isSafeSlug(slug)) {
    throw new Error(
      `Unsafe slug ${quote(slug)}. A slug must be lowercase words joined by single hyphens and at most ` +
        `${MAX_SLUG_LENGTH} characters, for example "harbour-lights".`,
    );
  }
...
function validateIdentity(record, where, spec, context, errors) {
  ...
  if (!isSafeSlug(record.slug)) {
    errors.push(
      // The length limit is named because a long title slugifies to something
      // that looks exactly like a valid slug: without it the message describes
      // the value the author is looking at and reads as nonsense.
      `${where}: slug must be lowercase words joined by single hyphens and at most ${MAX_SLUG_LENGTH} characters ` +
        `(found ${quote(record.slug)}, ${typeof record.slug === 'string' ? `${record.slug.length} characters` : 'not a string'}). ` +
        'Slugs become public detail URLs.',
    );
  } else {
...
function validateCuratedEntry(entry, where, allowedTypes, lookup, seenRefs, seenOrders, errors) {
  ...
  const strayType = allowedTypes.length === 1 && entry.type !== undefined && entry.type !== type;
  if (allowedTypes.length > 1 && !allowedTypes.includes(entry.type)) {
    ...
    errors.push(
      `${where}: type must be ${quote(type)} when supplied here (found ${quote(entry.type)}); this collection only holds ${quote(type)} entries.`,
    );
  } else {
    const spec = recordTypeSpec(type);
...
function loadDocument(document) {
  ...
  let next;
  try {
    next = safeClone(document);
  } catch {
    throw new Error('The portfolio document could not be cloned; it must be plain JSON with no circular references.');
  }
  for (const field of ARRAY_FIELDS) {
